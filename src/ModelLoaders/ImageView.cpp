...
ImageView::Model::Model(uint32_t framesInFlight, VulkanDevice *vulkanDevice) {
    ...
    m_texture.resize(framesInFlight);
...
void ImageView::Model::createEmptyTexture(uint32_t width, uint32_t height) {
    ...
    for (uint32_t i = 0; i < m_framesInFlight; ++i) {
        ...
        m_texture[i] = std::make_unique<TextureVideo>(width, height, m_vulkanDevice,
...
void ImageView::createDescriptors(bool useOffScreenImageRender) {
    ...
    VkDescriptorPoolCreateInfo poolCreateInfo = Populate::descriptorPoolCreateInfo(poolSizes, m_model->m_framesInFlight);
    CHECK_RESULT(vkCreateDescriptorPool(m_vulkanDevice->m_LogicalDevice, &poolCreateInfo, nullptr,
                                        &m_model->m_descriptorPool))
    ...
    for (uint32_t i = 0; i < m_model->m_framesInFlight; ++i) {
        ...
        writeDescriptorSets[0].dstSet = m_model->m_descriptors[i];
        writeDescriptorSets[0].dstBinding = 0;
        writeDescriptorSets[0].pBufferInfo = &m_renderUtils->uniformBuffers[i].bufferOne.m_DescriptorBufferInfo;

        writeDescriptorSets[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        writeDescriptorSets[1].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        writeDescriptorSets[1].descriptorCount = 1;
        writeDescriptorSets[1].dstSet = m_model->m_descriptors[i];
...
void ImageView::createGraphicsPipeline() {
    for (uint32_t i = 0; i < m_model->m_framesInFlight; ++i) {
        ...
        VkPipelineLayoutCreateInfo info = Populate::pipelineLayoutCreateInfo(&m_model->m_descriptorSetLayout[i]);
        ...
        CHECK_RESULT(
                vkCreatePipelineLayout(m_vulkanDevice->m_LogicalDevice, &info, nullptr, &m_model->m_pipelineLayout[i]))
        ...
        pipelineCI.layout = m_model->m_pipelineLayout[i];
        ...
        VkResult res = vkCreateGraphicsPipelines(m_vulkanDevice->m_LogicalDevice, VK_NULL_HANDLE, 1, &pipelineCI, nullptr,
                                                 &m_model->m_pipeline[i]);
        if (res != VK_SUCCESS)
...
void ImageView::draw(CommandBuffer *commandBuffer, uint32_t i) {
    ...
    vkCmdBindDescriptorSets(commandBuffer->buffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, m_model->m_pipelineLayout[i], 0,
                            1,
                            &m_model->m_descriptors[i], 0, nullptr);

    vkCmdBindPipeline(commandBuffer->buffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, m_model->m_pipeline[i]);
