...
namespace VkRender::PathTracer {
    ...
    void PhotonTracer::update(RenderSettings &renderSettings) {
        try {
            ...
            std::vector<int> debugData(imageSize, 0);

            // Create a SYCL buffer for the debug data.
            {
                sycl::buffer<int, 1> debugBuffer(debugData.data(), sycl::range<1>(imageSize));
                ...
                // Submit the kernel.
                queue.submit([&](sycl::handler &cgh) {
                    // Create an accessor for the debug buffer.
                    auto debugAcc = debugBuffer.get_access<sycl::access::mode::write>(cgh);
