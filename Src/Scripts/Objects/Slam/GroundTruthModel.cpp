...
void GroundTruthModel::setup() {
    ...
    for (const auto row: parser) {
        try {
            const auto [time, x, y, z, q1, q2, q3, q4] = row.cells(0, 3, 4, 5, 9, 10, 11,
                                                                   12); // indexes must be in ascending order
    ...
    std::vector<VkPipelineShaderStageCreateInfo> shaders2 = {{loadShader("Scene/spv/boxAdditional.vert",
                                                                         VK_SHADER_STAGE_VERTEX_BIT)},
                                                             {loadShader("Scene/spv/boxAdditional.frag",
                                                                         VK_SHADER_STAGE_FRAGMENT_BIT)}};
    size_t numTraces = 10;
    requestAdditionalBuffers(numTraces);
    m_Traces.resize(numTraces);
    for (int i = 0; i < numTraces; ++i) {
        m_Traces[i] = std::make_unique<glTFModel::Model>(renderUtils.device);
        m_Traces[i]->loadFromFile(Utils::getAssetsPath() + "Models/Box/glTF-Embedded/Box.gltf", renderUtils.device,
                                  renderUtils.device->m_TransferQueue, 1.0f);
        m_Traces[i]->createRenderPipeline(renderUtils, shaders2, additionalBuffersData[i],
...
void GroundTruthModel::draw(VkCommandBuffer commandBuffer, uint32_t i, bool b) {
    ...
    if (b) {
        for (const auto &item: m_Traces) {
            item->draw(commandBuffer, i);
        }
    }
...
void GroundTruthModel::update() {
    ...
    if (!shared->time.empty()) {
        ...
        glm::quat rot(gtPositions[index].orientation.w, gtPositions[index].orientation.x,
                      gtPositions[index].orientation.y, gtPositions[index].orientation.z);
        rotationMat = glm::toMat4(rot);
        translationMat = glm::rotate(translationMat, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
        translationMat = glm::translate(translationMat, glm::vec3(gtPositions[index].x, gtPositions[index].y, gtPositions[index].z));
    ...
    for (int i = 0; i < additionalBuffers.size(); i++) {
        auto &mvp = additionalBuffers[i]->mvp;
        mvp.model = glm::translate(glm::mat4(1.0f), glm::vec3((float) i, 0.0f, -5.0f)) * glm::rotate(glm::mat4(1.0f), glm::radians((float)drand48() * 360.0f), glm::vec3(1.0f, 1.0f, 1.0f)) * glm::scale(glm::mat4(1.0f), glm::vec3(0.1f, 0.1f, 0.1f));
        mvp.projection = renderData.camera->matrices.perspective;
        mvp.view = renderData.camera->matrices.view;

        auto &light = additionalBuffers[i]->light;
        light.objectColor = glm::vec4(0.25f, 0.25f, (float) i / 10.0f, 1.0f);
        light.lightColor = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
        light.lightPos = glm::vec4(glm::vec3(0.0f, -3.0f, 0.0f), 1.0f);
        light.viewPos = renderData.camera->m_ViewPos;
    }
}
