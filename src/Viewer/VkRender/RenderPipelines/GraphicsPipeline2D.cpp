...
namespace VkRender{
    ...
    void GraphicsPipeline2D::update(uint32_t currentFrame) {
        memcpy(m_renderData[currentFrame].fragShaderParamsBuffer.mapped,
               &m_fragParams, sizeof(ShaderValuesParams));
        memcpy(m_renderData[currentFrame].mvpBuffer.mapped,
               &m_vertexParams, sizeof(UBOMatrix));

    }
    void GraphicsPipeline2D::updateTransform(const TransformComponent &transform) {
    ...
    GraphicsPipeline2D::bind<MeshComponent>(
        ...
        size_t vertexBufferSize = modelComponent.m_vertices.size() * sizeof(Vertex);
