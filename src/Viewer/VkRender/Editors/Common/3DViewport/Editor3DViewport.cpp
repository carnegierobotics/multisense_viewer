...
namespace VkRender {
    ...
    Editor3DViewport::~Editor3DViewport() {
        ...
        if (m_pointCloudDescriptorSetLayout != VK_NULL_HANDLE) {
            vkDestroyDescriptorSetLayout(m_context->vkDevice().m_LogicalDevice, m_pointCloudDescriptorSetLayout,
                                         nullptr);
        }
    }
    ...
    void Editor3DViewport::onComponentRemoved(Entity entity, MeshComponent &meshComponent) {
        if (m_meshInstances.contains(entity.getUUID())) {
            m_meshInstances.erase(entity.getUUID());
        }
    ...
    void Editor3DViewport::onComponentRemoved(Entity entity, MaterialComponent &materialComponent) {
        if (m_materialInstances.contains(entity.getUUID())) {
            m_materialInstances.erase(entity.getUUID());
        }
    }
    ...
    void Editor3DViewport::allocatePerEntityDescriptorSet(uint32_t frameIndex, Entity entity) {
    ...
    void Editor3DViewport::updateGlobalUniformBuffer(uint32_t frameIndex, Entity entity) {
        ...
        if (entity.hasComponent<MeshComponent>()) {
            ...
            globalUBO.cameraPosition = cameraComponent.get().pose.pos;
