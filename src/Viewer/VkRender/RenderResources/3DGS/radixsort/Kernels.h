                ...
                if (blockIdx < gridDim - 1) {
                    ...
                    for (uint32_t i = threadIdx + (blockIdx * G_HIST_VEC_SIZE);
...

                        {
                            ...
                            s_wavesHistFirst_atomic.fetch_add(1u);
                            ...
                            s_wavesHistSec_atomic.fetch_add(1u);
                            ...
                            s_wavesHistThird_atomic.fetch_add(1u);
                            ...
                            s_wavesHistFourth_atomic.fetch_add(1u);
                        }
                        {
                            sycl::atomic_ref<uint32_t, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::local_space> s_wavesHistFirst_atomic(
        ...
        void operator()(sycl::nd_item<1> item) const {
            ...
            if (threadIdx == 0) {
                ...
                s_warpHistograms[BIN_PART_SIZE -
                        ...
                        m_indexBuffer[m_radixShift >> 3]).fetch_add(1u);
            ...
            if (partitionIndex == gridDim - 1) {
                ...
                for (uint32_t i = partStart; i < partEnd; i += blockDim) {
                    ...
                    for (uint32_t k = 0; k < RADIX_LOG; ++k) {
                        ...
                        uint32_t lane_id = item.get_sub_group().get_local_id();
                        uint32_t t_bit = t ? (1u << lane_id) : 0u;

                        // Perform a bitwise OR reduction across the subgroup
                        uint32_t mask = sycl::reduce_over_group(item.get_sub_group(), t_bit, sycl::bit_or<uint32_t>());

                        // Update warpFlags based on the condition
                        if (t) {
                            warpFlags &= mask;
                        } else {
                            warpFlags &= ~mask;
