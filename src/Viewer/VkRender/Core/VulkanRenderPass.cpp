...
namespace VkRender {
    ...
    void VulkanRenderPass::setupDepthOnlyRenderPass(const VulkanRenderPassCreateInfo *createInfo) {
        ...
        depthAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE; // Store depth for later use (e.g., disparity conversion)
        depthAttachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        depthAttachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
        ...
        depthSubpass.colorAttachmentCount = 0; // No color attachments
        ...
        depthDependencies[0].srcStageMask =
                VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
        depthDependencies[0].dstStageMask =
                VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
        depthDependencies[0].srcAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
        depthDependencies[0].dstAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
        depthDependencies[0].dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;

        depthDependencies[1].srcSubpass = 0;
        depthDependencies[1].dstSubpass = VK_SUBPASS_EXTERNAL;
        depthDependencies[1].srcStageMask = VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
        depthDependencies[1].dstStageMask = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
        depthDependencies[1].srcAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
        depthDependencies[1].dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
        depthDependencies[1].dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;
        ...
        renderPassInfo.pDependencies = depthDependencies;
        ...
        VALIDATION_DEBUG_NAME(m_logicalDevice,
                              reinterpret_cast<uint64_t>(m_renderPass), VK_OBJECT_TYPE_RENDER_PASS,
                              (createInfo->debugInfo + ":ColorRenderPass").c_str());
    }
}