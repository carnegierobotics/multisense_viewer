...
namespace VkRender::crl {
    ...
    static inline uint32_t InclusiveWarpScanCircularShift(sycl::sub_group &subGroup, uint32_t val) {
    ...
    static void DigitBinningPassFunc(sycl::queue &queue, sycl::buffer<uint32_t, 1> &sortBuffer,
                    ...
                    [=](sycl::nd_item<1> item) {
                        ...
                        if (partitionIndex < gridDim - 1) {
                            ...
                            }
                            {
                                ...
                                uint32_t res = sycl::select_from_group(subGroup, s_localHistogram[threadIdx - 1],
                                                                       1);
    ...
    public:
        RadixSorter(sycl::queue &_queue) {
            ...
            valuesAltBuffer = sycl::buffer<uint32_t, 1>(size);

            globalHistogramBuffer = sycl::buffer<uint32_t, 1>(sycl::range<1>(radix * radixPasses));
            firstPassHistogramBuffer = sycl::buffer<uint32_t, 1>(RADIX * binningThreadblocks);
            secPassHistogramBuffer = sycl::buffer<uint32_t, 1>(RADIX * binningThreadblocks);
            thirdPassHistogramBuffer = sycl::buffer<uint32_t, 1>(RADIX * binningThreadblocks);
            fourthPassHistogramBuffer = sycl::buffer<uint32_t, 1>(RADIX * binningThreadblocks);
            indexBuffer = sycl::buffer<uint32_t, 1>(RADIX);
    ...
    private:
        ...
        sycl::buffer<uint32_t, 1> sortAltBuffer{0};
        sycl::buffer<uint32_t, 1> valuesAltBuffer{0};
        sycl::buffer<uint32_t, 1> globalHistogramBuffer{0};
        sycl::buffer<uint32_t, 1> firstPassHistogramBuffer{0};
        sycl::buffer<uint32_t, 1> secPassHistogramBuffer{0};
