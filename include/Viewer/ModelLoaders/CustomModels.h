...
public:
    ...
    explicit CustomModelComponent(const VkRender::RenderUtils* renderUtils) {
        ...
        std::vector<VkPipelineShaderStageCreateInfo> shaders = {{CustomModelComponent::loadShader(vulkanDevice->m_LogicalDevice, "spv/grid.vert",
                                                                            VK_SHADER_STAGE_VERTEX_BIT, &vertModule)},
                                                                {CustomModelComponent::loadShader(vulkanDevice->m_LogicalDevice, "spv/grid.frag",
