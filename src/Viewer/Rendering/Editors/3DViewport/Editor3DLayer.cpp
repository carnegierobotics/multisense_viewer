...
namespace VkRender {
    ...
    void Editor3DLayer::onUIRender() {
            ...
            if (camera) {
                auto matrices = camera->getPerspectiveCamera()->matrices;
                ...
                float snap = 0;
                ImGuizmo::Manipulate(viewPtr, projectionPtr, ImGuizmo::TRANSLATE, ImGuizmo::WORLD,
                                     glm::value_ptr(transform));

                if (ImGuizmo::IsUsing()) {
                    transformComponent.setTransform(deltaMatrix);
